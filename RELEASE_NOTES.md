...
- Auto-isolate also handles a product standing on a narrower stand or box: scanning horizontal slices from the top down, the first slice whose width drops below 70% of the product's width (`PRODSPLAT_ISOLATE_NARROWING`) marks the product's bottom edge; everything below it goes, the stand's top inside the product's footprint goes, and the product's visible underside around the stand is kept. An optional `supportColor` render option (dashboard colour picker) additionally removes Gaussians of the stand's colour below that edge, for stands that are not clearly narrower. The isolation report (focus, orbit radius, table and bottom heights, radii, counts) is written into `render_manifest.json` under `isolation`.
